...
import { BlogTOC, slugParamProps } from "@/src/types/blog";
...
export async function generateMetadata({ params }: slugParamProps) {
  const blog = allBlogs.find((blog: Blog) => {
    return blog._raw.flattenedPath === params.slug;
  });
...
export default function BlogPage({ params }: slugParamProps) {
  const blog = allBlogs.find((blog: Blog) => {
    return blog._raw.flattenedPath === params.slug;
  });
  ...
  let imageList: ImageFieldData | string[] = [siteMetadata.socialBanner];
  if (blog.image) {
    imageList =
      typeof blog.image.filePath === "string"
        ? [siteMetadata.siteUrl + blog.image.filePath.replace("../public", "")]
  ...
  return (
    <>
      ...
      />
      <article>
        <div className="mt-8 text-center relative w-full h-[70vh] bg-dark">
          <div className="w-full z-10 flex flex-col items-center justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            ...
            <h1 className="inline-block my-6 font-semibold capitalize text-light text-5xl leading-normal relative w-5/6">
              {blog?.title}
            </h1>
          </div>
